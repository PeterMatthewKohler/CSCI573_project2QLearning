...
if q_Learning:  # Perform Q_Learning Algorithm to fill up Q table
...
# With Previous State info update Q Table using Q-Learning Algorithm Formula
def updateQTable(prevState, reward, action, discountFactor, learningRate):
    if prevState.Right != None and prevState.RightFront != None and prevState.Front != None and prevState.Left != None:
        ...
        # Update Q Table
        Q_TABLE[prev_index][action] = Q_TABLE[prev_index][action] + learningRate * (reward + discountFactor * max(Q_TABLE[current_index].values()) - Q_TABLE[prev_index][action])
...
def main():
    ...
    while not rospy.is_shutdown() and not trainingDone:
        ...
        # Enter loop for new episode
        while not terminate and not trainingDone:
            # If training, perform epsilon greedy actions
            if q_Learning:
                ...
                if r > epsilon: # Take Greedy Action
                    ...
                    action = doRandomAction(timestepDuration)
                    timestepsThisEpisode += 1
                # After action, get a reward
                reward = getReward()
                # Add reward to total accrued reward for this episode
                episodeReward += reward
                # Update Q table for this action
                updateQTable(prevState, reward, action, discountFactor, learningRate)
                ...
                timestep += 1
            # If not training, use Q_TABLE from qtable_storage.py
            else: 
